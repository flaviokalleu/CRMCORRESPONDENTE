...
import {
  ...
  DollarSign, ClipboardList, Database, Calendar, Gauge,
  Server, Globe, Monitor, PieChart as PieChartIcon,
  Wallet, Building2, KeyRound, Landmark, Home,
} from "lucide-react";
...
const DashboardAdministrador = () => {
  ...
  const fetchAll = useCallback(async (signal, { silent = false } = {}) => {
    ...
    try {
      const opts = { headers: authHeaders(), signal };
      ...
      if (monthlyRes.ok && weeklyRes.ok) {
        ...
        const labels = monthly.labels || MONTH_LABELS;
        const monthlyArr = monthly.monthlyData || Array(12).fill(0);
        ...
        const weekLabels = weekly.labels || WEEK_LABELS;
        const weeklyArr = weekly.weeklyData || Array(7).fill(0);
        const prevWeekArr = weekly.previousWeekData || Array(7).fill(0);

        setChartData({
          monthly: labels.map((l, i) => ({ name: l, clientes: monthlyArr[i] || 0, crescimento: monthlyGrowth[i] || 0 })),
          weekly: weekLabels.map((l, i) => ({ name: l, atual: weeklyArr[i] || 0, anterior: prevWeekArr[i] || 0 })),
        ...
        setChartData({
          monthly: MONTH_LABELS.map((l) => ({ name: l, clientes: 0, crescimento: 0 })),
          weekly: WEEK_LABELS.map((l) => ({ name: l, atual: 0, anterior: 0 })),
      ...
      // Portfólio: imóveis, aluguéis, proprietários e financeiro
      const [imoveisRes, alugueisRes, propRes, finRes] = await Promise.all([
        fetch(`${API_URL}/imoveis/?limit=200`, opts).catch(() => null),
        fetch(`${API_URL}/alugueis`, opts).catch(() => null),
        fetch(`${API_URL}/proprietarios`, opts).catch(() => null),
        fetch(`${API_URL}/fluxocaixa/dashboard`, opts).catch(() => null),
      ]);

      const imoveisData = imoveisRes?.ok ? await imoveisRes.json() : null;
      const imoveisList = Array.isArray(imoveisData) ? imoveisData : (imoveisData?.imoveis || imoveisData?.data || []);
      const alugueisList = alugueisRes?.ok ? await alugueisRes.json() : [];
      const proprietariosList = propRes?.ok ? await propRes.json() : [];
      const financeiro = finRes?.ok ? await finRes.json() : null;

      const situacaoCount = {};
      (imoveisList || []).forEach((im) => {
        const s = im.situacao_imovel || "disponivel";
        situacaoCount[s] = (situacaoCount[s] || 0) + 1;
      });
      const situacaoLabels = { disponivel: "Disponível", vendido: "Vendido", reservado: "Reservado" };
      const imoveisSituacaoData = Object.entries(situacaoCount).map(([k, v]) => ({ name: situacaoLabels[k] || k, value: v }));

      const alugados = (alugueisList || []).filter((a) => a.alugado).length;
      const disponiveisAluguel = (alugueisList || []).length - alugados;
      const alugueisStatusData = [
        { name: "Alugado", value: alugados },
        { name: "Disponível", value: disponiveisAluguel },
      ].filter((d) => d.value > 0);

      setPortfolio({
        totalImoveis: (imoveisList || []).length,
        totalAlugueis: (alugueisList || []).length,
        totalProprietarios: (proprietariosList || []).length,
        imoveisSituacaoData,
        alugueisStatusData,
        alugueisRecentes: (alugueisList || []).slice(0, 6),
        financeiro: financeiro || { totalReceitas: 0, totalDespesas: 0, lucro: 0, pendencias: 0 },
      });
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => fetchAll(controller.signal, { silent: true }), 30000);
    const vis = () => {
      isVisibleRef.current = document.visibilityState === "visible";
      if (isVisibleRef.current) fetchAll(controller.signal, { silent: true });
    };
  ...
  const derived = useMemo(() => {
    ...
    const rankingItems = (top5Usuarios || []).map((item) => {
      const u = item.user || {};
      return { id: u.id, name: `${u.first_name || ""} ${u.last_name || ""}`.trim() || "Usuário", value: item.clientes || 0 };
    });
    ...
    return {
      ...
      aguardando, rankingItems, crescimentoSemanal, crescimentoMensal, usuariosAtivosHoje,
      ...
      rendaAnalysis, pctAprov, pctRejeit, pctAguar, pieData,
    };
  }, [dashboardData]);
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden bg-caixa-gradient min-h-screen">
      {/* -- Header -- */}
      <div
        className="flex-shrink-0 flex items-center justify-between px-4 sm:px-5 py-3 border-b backdrop-blur-md"
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-white/50 capitalize">{today}</p>
          </div>
        </div>
        <div className="flex items-center gap-2.5">
          <span className="hidden sm:inline text-[10px] text-white/35">Atualizado às {lastUpdatedLabel}</span>
          <button
            onClick={() => fetchAll()}
            disabled={refreshing}
            className="flex items-center justify-center w-8 h-8 rounded-lg text-white/60 transition-colors hover:bg-white/10 hover:text-white disabled:opacity-50"
      ...
      <motion.div className="flex-1 overflow-y-auto p-3 sm:p-5 space-y-4" initial="hidden" animate="show" variants={stagger}>
        ...
        <motion.div variants={stagger} className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5">
          <KPICard index={0} title="Total Clientes" value={fmtNum(d.totalClientes)} sub={`${d.clientesHoje} hoje`}
            icon={<Users className="w-4 h-4" />} accent="#3b82f6" trend={d.crescimentoMensal} />
          ...
          <KPICard index={3} title="Aprovados" value={fmtNum(d.clientesAprovados)} sub={`${d.pctAprov}%`}
            icon={<CheckCircle className="w-4 h-4" />} accent="#10b981" />
          <KPICard index={4} title="Reprovados" value={fmtNum(d.clientesReprovados)} sub={`${d.pctRejeit}%`}
            icon={<XCircle className="w-4 h-4" />} accent="#ef4444" />
          <KPICard index={5} title="Online" value={fmtNum(systemStats?.usuariosRecentes || d.usuariosAtivosHoje)} sub="ativos agora"
            icon={<Signal className="w-4 h-4" />} accent="#06b6d4" />
        </motion.div>

        {/* --- CONTROLE DETALHADO --- */}
        <SectionHeader icon={<Activity className="w-3.5 h-3.5" />} title="Controle Detalhado" subtitle="Métricas de crescimento e performance" />

        <motion.div variants={stagger} className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2.5">
          <KPICard index={0} title="Hoje" value={fmtNum(d.clientesHoje)} sub="cadastros"
          ...
          <KPICard index={2} title="Este Mês" value={fmtNum(d.clientesEsteMes)} sub={`ant: ${d.clientesMesAnterior}`}
            icon={<Activity className="w-4 h-4" />} accent="#06b6d4" trend={d.crescimentoMensal} />
          <KPICard index={3} title="No Ano" value={fmtNum(chartData.totalYear)} sub={`média: ${chartData.averageMonth}/mês`}
          ...
          <KPICard index={7} title="Usuários" value={fmtNum(d.performance?.totalUsuarios || systemStats?.totalUsuarios || 0)} sub="no sistema"
            icon={<UserCheck className="w-4 h-4" />} accent="#ec4899" />
        </motion.div>

        {/* --- ANÁLISE FINANCEIRA (renda dos clientes) --- */}
        <SectionHeader icon={<DollarSign className="w-3.5 h-3.5" />} title="Análise Financeira" subtitle="Perfil de renda dos clientes" />
        ...
        <motion.div variants={stagger} className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
          ...
          <KPICard index={3} title="Com Renda" value={fmtNum(d.rendaAnalysis?.clientesComRenda || 0)} sub="clientes informaram"
            icon={<Wallet className="w-4 h-4" />} accent="#8b5cf6" />
        </motion.div>

        {/* --- PERFORMANCE + SISTEMA --- */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
          <motion.div variants={fadeUp} className="rounded-xl p-3 backdrop-blur-md" style={{ backgroundColor: CARD, border: `1px solid ${BORDER}` }}>
          ...
          <motion.div variants={fadeUp} className="rounded-xl p-3 backdrop-blur-md" style={{ backgroundColor: CARD, border: `1px solid ${BORDER}` }}>
            <div className="flex items-center gap-2 mb-2.5">
              <div className="w-5 h-5 rounded-md flex items-center justify-center" style={{ background: "linear-gradient(135deg, #06b6d4, #22d3ee)" }}>
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <MiniStat icon={<Zap className="w-3 h-3" />} label="Efic. Geral" value={activityMetrics?.efficiency?.toFixed(1) || "0"} accent="#8b5cf6" />
              <MiniStat icon={<Monitor className="w-3 h-3" />} label="Online Agora" value={fmtNum(activityMetrics?.onlineUsers || 0)} accent="#ec4899" />
            </div>
          </motion.div>
        </div>

        {/* --- GRÁFICOS --- */}
        <SectionHeader icon={<TrendingUp className="w-3.5 h-3.5" />} title="Gráficos" subtitle="Tendências, comparativos e distribuição" />
        ...
        <motion.div variants={stagger} className="grid grid-cols-1 lg:grid-cols-2 gap-3">
          ...
          <ChartCard index={2} title="Status dos Clientes" sub="Distribuição por status de aprovação" icon={<PieChartIcon className="w-3 h-3" />}>
              ...
              {d.pieData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    ...
                    <Pie data={d.pieData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} innerRadius={45}
                      paddingAngle={3} cornerRadius={4} strokeWidth={0}
                      label={({ name, percent }) => `${name} ${((percent || 0) * 100).toFixed(0)}%`} labelLine={false}>
                      {d.pieData.map((_, i) => <Cell key={i} fill={`url(#adPie-${i})`} />)}
                    </Pie>
                    <Tooltip content={<CustomTooltip />} />
                  </PieChart>
                </ResponsiveContainer>
              ) : (
                <p className="w-full text-center text-xs text-white/40">Sem dados suficientes</p>
              )}
            </div>
          </ChartCard>

          <ChartCard index={3} title="Atividade por Dia" sub="Clientes cadastrados por dia da semana" icon={<Calendar className="w-3 h-3" />}>
            <div className="h-52 px-1">
              ...
              <div className="mt-2 pt-2 border-t flex items-center justify-between" style={{ borderColor: BORDER }}>
                ...
                <span className="text-[10px] font-bold text-white/70">
                  {chartData.weekly[chartData.weekly.reduce((best, x, i, arr) => (x.atual > arr[best].atual ? i : best), 0)]?.name || "-"}
                </span>
              </div>
            </div>
          </ChartCard>
        ...
        {portfolio && (
          <>
            <SectionHeader icon={<Building2 className="w-3.5 h-3.5" />} title="Portfólio Imobiliário" subtitle="Imóveis, aluguéis, proprietários e financeiro" />

            <motion.div variants={stagger} className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5">
              <KPICard index={0} title="Imóveis" value={fmtNum(portfolio.totalImoveis)} sub="à venda"
                icon={<Home className="w-4 h-4" />} accent="#3b82f6" />
              <KPICard index={1} title="Aluguéis" value={fmtNum(portfolio.totalAlugueis)} sub="em locação"
                icon={<KeyRound className="w-4 h-4" />} accent="#F97316" />
              <KPICard index={2} title="Proprietários" value={fmtNum(portfolio.totalProprietarios)}
                icon={<Landmark className="w-4 h-4" />} accent="#8b5cf6" />
              <KPICard index={3} title="Receitas" value={fmtR$(portfolio.financeiro.totalReceitas)}
                icon={<TrendingUp className="w-4 h-4" />} accent="#10b981" />
              <KPICard index={4} title="Despesas" value={fmtR$(portfolio.financeiro.totalDespesas)}
                icon={<TrendingDown className="w-4 h-4" />} accent="#ef4444" />
              <KPICard index={5} title="Lucro" value={fmtR$(portfolio.financeiro.lucro)} sub={`${portfolio.financeiro.pendencias} pendências`}
                icon={<Wallet className="w-4 h-4" />} accent={portfolio.financeiro.lucro >= 0 ? "#10b981" : "#ef4444"} />
            </motion.div>

            <motion.div variants={stagger} className="grid grid-cols-1 lg:grid-cols-3 gap-3">
              <ChartCard index={0} title="Situação dos Imóveis" sub={`${portfolio.totalImoveis} cadastrados`} icon={<Home className="w-3 h-3" />}>
                <div className="h-48 flex items-center">
                  {portfolio.imoveisSituacaoData.length > 0 ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={portfolio.imoveisSituacaoData} dataKey="value" nameKey="name" cx="50%" cy="50%"
                          outerRadius={70} innerRadius={38} paddingAngle={3} cornerRadius={4} strokeWidth={0}
                          label={({ name, percent }) => `${name} ${((percent || 0) * 100).toFixed(0)}%`} labelLine={false}>
                          {portfolio.imoveisSituacaoData.map((_, i) => <Cell key={i} fill={PALETTE[i % PALETTE.length]} />)}
                        </Pie>
                        <Tooltip content={<CustomTooltip />} />
                      </PieChart>
                    </ResponsiveContainer>
                  ) : (
                    <p className="w-full text-center text-xs text-white/40">Sem imóveis cadastrados</p>
                  )}
                </div>
              </ChartCard>

              <ChartCard index={1} title="Status dos Aluguéis" sub={`${portfolio.totalAlugueis} imóveis para locação`} icon={<KeyRound className="w-3 h-3" />}>
                <div className="h-48 flex items-center">
                  {portfolio.alugueisStatusData.length > 0 ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={portfolio.alugueisStatusData} dataKey="value" nameKey="name" cx="50%" cy="50%"
                          outerRadius={70} innerRadius={38} paddingAngle={3} cornerRadius={4} strokeWidth={0}
                          label={({ name, percent }) => `${name} ${((percent || 0) * 100).toFixed(0)}%`} labelLine={false}>
                          <Cell fill="#F97316" />
                          <Cell fill="#10b981" />
                    ...
                    <p className="w-full text-center text-xs text-white/40">Sem imóveis para locação</p>
                  )}
                </div>
              </ChartCard>

              <motion.div variants={fadeUp} className="rounded-xl p-4 backdrop-blur-md" style={{ backgroundColor: CARD, border: `1px solid ${BORDER}` }}>
                <div className="flex items-center gap-2 mb-3">
                  <div className="w-6 h-6 rounded-md flex items-center justify-center" style={{ background: "linear-gradient(135deg, #F97316, #EA580C)" }}>
                    <KeyRound className="w-3.5 h-3.5 text-white" />
                  </div>
                  <p className="text-xs font-bold text-white">Imóveis em Locação</p>
                </div>
                <div className="space-y-1.5">
                  {portfolio.alugueisRecentes.length > 0 ? (
                    portfolio.alugueisRecentes.map((a) => (
                      <div key={a.id} className="flex items-center justify-between px-2.5 py-2 rounded-lg" style={{ backgroundColor: "rgba(255,255,255,0.03)" }}>
                        <div className="min-w-0">
                          <p className="truncate text-[11px] font-medium text-white">{a.nome_imovel || `Imóvel #${a.id}`}</p>
                          <p className="text-[9px] text-white/40">{fmtR$(a.valor_aluguel)}/mês</p>
                        </div>
                        <span className="flex-shrink-0 text-[9px] font-bold px-2 py-0.5 rounded-full"
                          style={{ backgroundColor: a.alugado ? "rgba(249,115,22,0.15)" : "rgba(16,185,129,0.15)", color: a.alugado ? "#F97316" : "#10b981" }}>
                          {a.alugado ? "Alugado" : "Disponível"}
                        </span>
                      </div>
                    ))
                  ) : (
                    <p className="text-xs text-center py-3 text-white/40">Sem imóveis cadastrados</p>
                  )}
                </div>
              </motion.div>
            </motion.div>
        ...
        {/* --- RANKING + ALERTAS + RESUMO --- */}
        <SectionHeader icon={<Trophy className="w-3.5 h-3.5" />} title="Ranking & Alertas" subtitle="Top performers e notificações" />
        ...
        <motion.div variants={stagger} className="grid grid-cols-1 lg:grid-cols-3 gap-3">
          ...
          <motion.div variants={fadeUp} className="rounded-xl p-4 space-y-3 backdrop-blur-md" style={{ backgroundColor: CARD, border: `1px solid ${BORDER}` }}>
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded-md flex items-center justify-center" style={{ background: "linear-gradient(135deg, #ef4444, #f87171)" }}>
            ...
            <div className="space-y-2">
              {d.aguardando.length > 0 && (
                <div className="flex items-center justify-between px-2.5 py-2 rounded-lg" style={{ backgroundColor: "rgba(234,179,8,0.08)" }}>
                  ...
                  <span className="text-xs font-bold px-2 py-0.5 rounded-full" style={{ backgroundColor: "rgba(234,179,8,0.15)", color: "#eab308" }}>{d.aguardando.length}</span>
                </div>
              )}
              {d.clientesReprovados > 0 && (
                <div className="flex items-center justify-between px-2.5 py-2 rounded-lg" style={{ backgroundColor: "rgba(239,68,68,0.08)" }}>
                  ...
                  <span className="text-xs font-bold px-2 py-0.5 rounded-full" style={{ backgroundColor: "rgba(239,68,68,0.15)", color: "#ef4444" }}>{d.clientesReprovados}</span>
                </div>
              )}
          ...
          <motion.div variants={fadeUp} className="rounded-xl p-4 space-y-3 backdrop-blur-md" style={{ backgroundColor: CARD, border: `1px solid ${BORDER}` }}>
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded-md flex items-center justify-center" style={{ background: "linear-gradient(135deg, #3b82f6, #60a5fa)" }}>
            ...
            <div className="space-y-1.5">
              {[
                ...
                <div key={i} className="flex items-center justify-between px-2 py-1.5 rounded-md" style={{ backgroundColor: i % 2 === 0 ? "transparent" : "rgba(255,255,255,0.02)" }}>
        ...
        {/* --- TABELA AGUARDANDO --- */}
        <SectionHeader icon={<Database className="w-3.5 h-3.5" />} title="Clientes Aguardando Aprovação" subtitle={`${d.aguardando.length} pendentes`} />

        <motion.div variants={fadeUp} className="rounded-2xl overflow-hidden backdrop-blur-md" style={{ backgroundColor: CARD, border: `1px solid ${BORDER}` }}>
                    ...
                    return (
                      <motion.tr key={cliente.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                        ...
                        <td className="py-2.5 px-4">
                          <div className="flex items-center gap-2">
                            <div className="w-7 h-7 rounded-lg flex items-center justify-center text-[10px] font-bold text-white flex-shrink-0" style={{ background: ACCENT_GRADIENT }}>
                        ...
                        <td className="py-2.5 px-4">
                          <span className="text-[10px] font-bold px-2 py-0.5 rounded-md" style={{ backgroundColor: sc.bg, color: sc.color }}>{getStatusDisplay(cliente.status)}</span>
                        </td>
                        <td className="py-2.5 px-4 text-[11px] text-white/50">{cliente.created_at ? new Date(cliente.created_at).toLocaleDateString("pt-BR") : "-"}</td>
                        <td className="py-2.5 px-4">
                          <span className="text-[10px] font-bold px-2 py-0.5 rounded-md" style={{ backgroundColor: `${urgencia}15`, color: urgencia }}>{dias}d</span>
                        </td>
                      </motion.tr>
                    );
                  })
