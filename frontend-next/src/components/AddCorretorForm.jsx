...
// Portado de frontend/src/components/AddCorretor.jsx (simplificado: sem
// upload de foto — ver docs-wiring-clientes.md). Envia JSON em vez de
// multipart/form-data já que não há arquivo.
const initialState = {
  username: "",
  email: "",
  first_name: "",
...
export function AddCorretorForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.username || !formData.email || !formData.first_name || !formData.last_name || !formData.telefone || !formData.password) {
    ...
    try {
      ...
      const res = await fetch("/api/backend/corretor", {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        throw new Error(data.message || data.error || `Erro ao criar corretor (${res.status})`);
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="crm-form max-w-4xl space-y-4 rounded-xl border border-cx-border bg-cx-surface p-6">
      <FormIntro title="Perfil do corretor" description="Preencha os dados pessoais, profissionais e de acesso. Campos com * são obrigatórios." />
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Field label="Telefone *">
          <input name="telefone" value={formData.telefone} onChange={handleChange} required className="input" placeholder="(00) 00000-0000" />
        </Field>
        <Field label="CRECI">
          <input name="creci" value={formData.creci} onChange={handleChange} className="input" />
        </Field>
