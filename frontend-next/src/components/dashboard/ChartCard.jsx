...
export function ChartCard({ title, subtitle, action, live, tag = "blue", children, className = "" }) {
  const dotColor = tag === "orange" ? CHART_COLORS.orange : CHART_COLORS.blue;
    ...
    <div className={`rounded-lg border border-white/10 bg-white/[0.04] p-3 ${className}`}>
      <div className="mb-2 flex items-start justify-between gap-3">
        <div className="flex items-center gap-1.5">
          ...
          <div>
            <h2 className="text-[0.7rem] font-semibold uppercase tracking-[0.08em] text-white/60">{title}</h2>
            {subtitle && <p className="font-tabular mt-0.5 text-xs text-white/70">{subtitle}</p>}
          </div>
        </div>
        <div className="flex items-center gap-2">
          {live && (
            <span className="inline-flex items-center gap-1.5 text-[0.65rem] font-medium uppercase tracking-wide text-emerald-400/80">
