  ...
  return (
    <div className="rounded-lg border border-cx-border bg-cx-surface px-3 py-2 text-xs shadow-xl">
      <p className="font-semibold text-cx-text">
        {p.value.toLocaleString("pt-BR")} <span className="font-normal text-cx-muted">clientes</span>
      </p>
    </div>
  );
}
...
export function TopUsuariosChart({ usuarios }) {
  if (!usuarios?.length) {
    return <p className="py-8 text-center text-xs text-cx-muted">Sem dados de ranking ainda.</p>;
  }
    ...
    <ResponsiveContainer width="100%" height={Math.max(140, data.length * 32)}>
      <BarChart data={data} layout="vertical" margin={{ top: 0, right: 16, left: 0, bottom: 0 }} barGap={2}>
        ...
        <Bar dataKey="clientes" radius={[0, 4, 4, 0]} maxBarSize={22}>
          {data.map((_, i) => (
            <Cell key={i} fill={i === 0 ? CHART_COLORS.white : CHART_COLORS.navy} fillOpacity={i === 0 ? 1 : 1 - i * 0.14} />
          ))}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
